    ...
    return(
        <div>
            ...
            className="mx-auto w-[250px] border-[#fff] rounded-md group hover:bg-[#fff] hover:scale-110 lg: hover:border-0 hover:p-4 hover:shadow-xl transition-all origin-center duration-400 max-lg:mb-6">
                <div id="imgBox" className="w-full aspect-[4/3] border-10 rounded-md max-lg:border-6 max-md:border-4 border-[#fff] group-hover:border-0 transition-all origin-center duration-400">
                    <div id="img" 
                    className="rounded-md bg-cover-set w-full absolute inset-0 "
                    style={{backgroundImage: `url(${img}), url(${defaultImg})`}}/>
                </div>
                <h1 className="m-0 text-2xl line-clamp-1 leading-loose group-hover:line-clamp-2 group-hover:leading-snug">{title}</h1>
                <p className="mt-1 text-sm text-gray-500">
                    {new Date(startDate).toLocaleDateString()} - {new Date(endDate).toLocaleDateString()}
                </p>
                <div id="text" 
                className=" transform translate-x-[20%] opacity-0 invisible group-hover:visible group-hover:opacity-100 group-hover:translate-0 transition-all duration-20 overflow-hidden"
                >
